...
including exactly which messages missed and why, is in
[`eval/eval_report.md`](eval/eval_report.md) (regenerated by
`eval/run_eval.py`, checked in).
...
- **Telegram channel: verified live, end-to-end.** The bot
  ...
  for the full trail.
- **Confidence calibration check** (spec Phase 5 stretch): measuring whether
  "the model said X% confident" actually corresponds to X% correct in
  practice. Not built. Would be the single most impressive addition, per
  the original spec, precisely because almost nobody building these demos
  does it.
