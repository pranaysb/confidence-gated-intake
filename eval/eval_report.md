...
- **Auto-routed**: 14 / 30
- **False-confidence count**: 3 (auto-routed with confidence ≥ 0.7, but wrong)
- **False-confidence rate, of auto-routed messages**: 21.4%
- **False-confidence rate, of all messages**: 10.0%
...
- **clear_005** (clear): confidence=0.69, consistency=0.92, completeness=0.75, auto_routed=False. Explicit 'no timeline pressure' -> low urgency, all fields clean.
...
- **clear_013** (clear): confidence=0.62, consistency=0.83, completeness=0.75, auto_routed=False. 'totally flexible on timing' is an explicit low-urgency signal.
...
**Expected Calibration Error (ECE): 0.402**

| Confidence range | Messages | Mean confidence | Actual accuracy | Gap |
|---|---|---|---|---|
| 0.0–0.2 | 7 | 0.0% | 85.7% | +85.7% |
| 0.2–0.4 | 2 | 24.0% | 100.0% | +76.0% |
| 0.4–0.6 | 4 | 49.0% | 50.0% | +1.0% |
| 0.6–0.8 | 3 | 66.7% | 0.0% | -66.7% |
| 0.8–1.0 | 14 | 96.4% | 78.6% | -17.9% |
...
**Reading the 0.6–0.8 bucket (largest over-confidence gap, -66.7%):** only 3 message(s) in this bucket, so treat as a signal to watch rather than a conclusion -- but this is the dangerous direction (confidence overstating actual correctness), and it's driven by the same kind of borderline urgency-label disagreement documented in "What broke it, and how" above, not a new failure mode.
